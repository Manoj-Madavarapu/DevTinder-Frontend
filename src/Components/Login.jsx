...
const Login = () => {
  ...
  return (
    <div className='body_bg h-screen'>
      <Nav></Nav>
      <h1 className='absolute left-4 top-23 md:left-20 md:top-25 font-bold cursor-pointer text-[16px] text-sm sm:text-base back px-4 !text-white'onClick={()=>navigate("/")}><i className="fa-solid fa-arrow-left !text-white"></i> Go back to Home Page</h1>
        ...
        <div className="card bg-base-300 w-96 h-100  flex flex-col justify-center py-8 card_shadow pl-3">
              <div className="card-body bg-black !text-white ">
                <fieldset className="fieldset">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input text-white" placeholder="Enter your Email" />
                  <p className="label"></p>
                </fieldset>
                <fieldset className="fieldset">
                  ...
                  <input type={showPassword?"text":"password"} value={password} onChange={(e) => setPassword(e.target.value)} className="input mb-6 text-white " placeholder="Enter your Password" />
                ...
                <p className='mt-2 text-white'>Don't have an Account? <span className='underline font-bold text-blue-500 cursor-pointer' onClick={()=>navigate("/devTinder/sign-up")}> Sign Up</span></p>
              </div>
        </div>
      </div>
    </div>
